...
test:
    PYTHONPATH=src uv run python -m pytest tests/domain/ tests/application/ tests/infrastructure/ -q

# Run architecture tests only
test-arch:
    PYTHONPATH=src uv run python -m pytest tests/architecture/ -v
...
coverage:
    PYTHONPATH=src uv run python -m pytest --cov=src/atomicguard --cov-report=term-missing tests/domain/ tests/application/ tests/infrastructure/

# ─── Smoke Tests ─────────────────────────────────────────

# Run all zero-dependency smoke tests
smoke:
    PYTHONPATH=src uv run python -m examples.basics.01_mock
    PYTHONPATH=src uv run python -m examples.basics.05_versioned_env
