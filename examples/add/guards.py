...
from .models import ArtifactManifest, GatesExtractionResult, ProjectConfig, TestSuite
...
class ConfigGuard(GuardInterface):
    ...
    Per the paper: This guard validates Ω (Global Constraints) before
    they are added to context.ambient.constraints.
    """
    ...
    def validate(self, artifact: Artifact, **_deps: Any) -> GuardResult:
        ...
        if not config.source_root:
            ...
            return GuardResult(
                ...
                "or infer from layer paths (e.g., 'src/myapp/domain/' → 'src/myapp').",
            )
