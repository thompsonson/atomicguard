...
This example demonstrates automated import validation using `ImportGuard` before human review. Unlike `tdd_human_review`, this example catches common LLM mistakes (like missing `import pytest`) automatically.

## Overview

The key improvement over `tdd_human_review`:

| Aspect | tdd_human_review | tdd_import_guard |
|--------|------------------|------------------|
| Import validation | Human must catch | **Automated guard catches** |
| Guard chain | SyntaxGuard → HumanReviewGuard | SyntaxGuard → **ImportGuard** → HumanReviewGuard |
| Failure mode | Human error → workflow fails | Guard catches → LLM retries |
...
### Comparison: What Happens Without ImportGuard

In `tdd_human_review`, if the human approves tests with missing imports:

1. Human sees tests, doesn't notice missing `import pytest`
2. Human approves → tests are locked in
...
6. Workflow fails after max attempts

**The ImportGuard prevents this by catching the error before human review.**
