#!/usr/bin/env python3
"""
TDD Workflow with Human Review Guard - JSON Configuration.

This example demonstrates a configuration-driven TDD workflow where:
1. Prompts and workflow structure are loaded from JSON files
2. Step 1: Generate tests with SyntaxGuard + HumanReviewGuard
3. Step 2: Generate implementation validated by DynamicTestGuard

The human guard ensures test quality before implementation begins,
following the TDD principle of writing good tests first.

Setup:
    1. Install Ollama: https://ollama.ai/
    2. Pull a model: ollama pull qwen2.5-coder:7b
    3. Run this script: python examples/tdd_human_review/run.py
...
def setup_logging(
    ...
    """
    logger = logging.getLogger("tdd_human_review")
...
VALID_GUARD_TYPES = ["syntax", "human", "dynamic_test", "composite"]
...
def create_workflow(
    ...
    model = model_override or workflow_config.get("model", "qwen2.5-coder:7b")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="TDD Workflow with Human Review Guard",
...
def main() -> int:
    """
    Execute TDD workflow with human test review from JSON configuration.
    ...
    logger.info("Starting TDD workflow with human review")
    ...
    model = args.model or workflow_config.get("model", "qwen2.5-coder:7b")
