    ...
    def test_raises_without_api_key_or_env(self) -> None:
        ...
        with patch.dict("os.environ", {}, clear=True):
            with pytest.raises(ValueError, match="HF_TOKEN"):
                HuggingFaceGenerator(
                    HuggingFaceGeneratorConfig(api_key=None)
                )

    def test_uses_env_token(self) -> None:
        """Init reads HF_TOKEN from environment."""
        pytest.importorskip("huggingface_hub")
        with patch.dict("os.environ", {"HF_TOKEN": "hf_env_token"}):
            gen = HuggingFaceGenerator(HuggingFaceGeneratorConfig())
            assert gen._version_counter == 0

    def test_uses_explicit_api_key(self) -> None:
        """Init prefers explicit api_key over env var."""
        pytest.importorskip("huggingface_hub")
        gen = HuggingFaceGenerator(
            HuggingFaceGeneratorConfig(api_key="hf_explicit")
        )
        assert gen._model == "Qwen/Qwen2.5-Coder-32B-Instruct"

    def test_legacy_kwargs_init(self) -> None:
        """Init works with legacy kwargs."""
        pytest.importorskip("huggingface_hub")
        gen = HuggingFaceGenerator(
            api_key="hf_test", model="custom/model"
        )
    ...
    def test_raises_without_huggingface_hub(self) -> None:
        ...
        with patch.dict("sys.modules", {"huggingface_hub": None}):
            with pytest.raises(ImportError, match="huggingface_hub"):
                HuggingFaceGenerator(
                    HuggingFaceGeneratorConfig(api_key="hf_test")
                )
...
"""
    ...
    def test_code_starting_with_def(
        self, generator: HuggingFaceGenerator
    ) -> None:
...
More text"""
    ...
    def test_code_starting_with_import(
        self, generator: HuggingFaceGenerator
    ) -> None:
...
def main():
    ...
    def test_code_starting_with_class(
        self, generator: HuggingFaceGenerator
    ) -> None:
...
class TestHuggingFaceGeneratorGenerate:
    ...
    def test_generate_returns_artifact(
        self, generator: HuggingFaceGenerator, sample_context  # noqa: ANN001
    ) -> None:
    ...
    def test_generate_sets_workflow_id(
        ...
        artifact = generator.generate(
            sample_context, workflow_id="wf-123"
        )
    ...
    def test_generate_sets_action_pair_id(
        ...
        artifact = generator.generate(
            sample_context, action_pair_id="ap-test"
        )
    ...
    def test_generate_handles_empty_response(
        ...
        generator._client.chat_completion.return_value.choices[
            0
        ].message.content = ""
        artifact = generator.generate(sample_context)
