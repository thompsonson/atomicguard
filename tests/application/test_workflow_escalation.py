...
class FailNTimesThenPassGuard(GuardInterface):
    ...
    def validate(self, _artifact: Artifact, **_deps: Artifact) -> GuardResult:
        ...
        if self._call_count <= self._fail_count:
            return GuardResult(
                passed=False, feedback=f"{self._feedback}: attempt {self._call_count}"
            )
        return GuardResult(passed=True, feedback="")
...
class TestWorkflowStepWithEscalation:
    ...
    def test_add_step_with_r_patience(self) -> None:
        ...
        pair = ActionPair(
            generator=gen, guard=AlwaysPassGuard(), prompt_template=_TEMPLATE
        )
        workflow = Workflow(
            artifact_dag=InMemoryArtifactDAG(), rmax=5
        )  # rmax > r_patience for valid config
    ...
    def test_add_step_with_escalate_feedback_to(self) -> None:
        """add_step stores escalate_feedback_to targets."""
        gen = MockGenerator(responses=["x = 1"])
        pair = ActionPair(
            generator=gen, guard=AlwaysPassGuard(), prompt_template=_TEMPLATE
        )
        ...
        workflow.add_step("g_impl", pair, escalate_feedback_to=("g_test", "g_analysis"))

        assert workflow._steps[0].escalate_feedback_to == ("g_test", "g_analysis")
...
class TestContextInjection:
    ...
    def test_inject_failure_context_stores_summary(self) -> None:
        ...
        workflow._inject_failure_context(
            "ap_analysis", "## Failure Summary\nTest failed"
        )
...
class TestEscalationTrigger:
    ...
    def test_escalation_after_stagnation(self) -> None:
        ...
        workflow.add_step(
            ...
            r_patience=2,
            e_max=1,
            escalate_feedback_to=("ap_analysis",),
        )
...
class TestStagnationVsFatalEscalation:
    ...
    def test_fatal_guard_bypasses_backtracking(self) -> None:
        ...
        class FatalGuard(GuardInterface):
            def validate(self, _a: Artifact, **_d: Artifact) -> GuardResult:
                return GuardResult(
                    passed=False, feedback="FATAL: Security issue", fatal=True
                )
        ...
        pair2 = ActionPair(
            generator=gen2, guard=FatalGuard(), prompt_template=_TEMPLATE
        )
        ...
        workflow.add_step(
            ...
            r_patience=2,
            e_max=2,
            escalate_feedback_to=("ap_analysis",),  # Should be ignored for fatal
        )
    ...
    def test_e_max_exhausted_promotes_to_human_escalation(self) -> None:
        ...
        class AlwaysSimilarFailGuard(GuardInterface):
            def validate(self, _a: Artifact, **_d: Artifact) -> GuardResult:
                return GuardResult(
                    passed=False, feedback="Test failed: same error pattern"
                )
        ...
        pair2 = ActionPair(
            generator=gen2, guard=AlwaysSimilarFailGuard(), prompt_template=_TEMPLATE
        )
...
class TestEscalationBudgetEnforcement:
    ...
    def test_escalation_count_increments_per_step(self) -> None:
        ...
        workflow = Workflow(artifact_dag=InMemoryArtifactDAG())
        workflow.add_step(
            "ap_step1", pair, r_patience=2, e_max=3, escalate_feedback_to=()
        )
        workflow.add_step(
            "ap_step2", pair, r_patience=2, e_max=5, escalate_feedback_to=()
        )
...
class TestInvariantValidation:
    ...
    def test_invalid_escalation_target_raises_on_execute(self) -> None:
        ...
        workflow = Workflow(artifact_dag=InMemoryArtifactDAG(), rmax=5)
        workflow.add_step("step1", pair, escalate_feedback_to=("nonexistent",))
    ...
    def test_valid_escalation_targets_allowed(self) -> None:
        ...
        workflow.add_step(
            "step2", pair, requires=("step1",), escalate_feedback_to=("step1",)
        )
...
class TestGuardSpecificEscalation:
    ...
    def test_workflow_step_escalation_by_guard(self) -> None:
        ...
        step = WorkflowStep(
            ...
            requires=(),
            deps=(),
            escalate_feedback_by_guard=escalation_map,
        )

        assert step.escalate_feedback_by_guard["SyntaxGuard"] == ("syntax_fix",)
        assert step.escalate_feedback_by_guard["TypeGuard"] == ("type_fix",)
    ...
    def test_add_step_with_escalation_by_guard(self) -> None:
        ...
        workflow.add_step(
            ...
            r_patience=2,
            e_max=2,
            escalate_feedback_to=("upstream",),
            escalate_feedback_by_guard={"Guard1": ("target1",)},
        )

        step = workflow.get_step("test_step")
        assert step.r_patience == 2
        assert step.e_max == 2
        assert step.escalate_feedback_to == ("upstream",)
        assert step.escalate_feedback_by_guard["Guard1"] == ("target1",)
