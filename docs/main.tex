...
\usepackage{textcomp}
\usepackage{tikz}
\usetikzlibrary{shapes, arrows, positioning, calc, shadows, fit, backgrounds, positioning, arrows.meta}
...
Modern software engineering has evolved sophisticated practices for managing complexity and uncertainty: CFEngine for infrastructure convergence, XP for iterative development, CI/CD for continuous validation, and Domain-Driven Design for semantic boundaries. These practices share a common threadâ€”they provide deterministic frameworks for managing inherently unpredictable processes.

Large Language Models present a parallel challenge: substantial capability with stochastic behavior. While "Attention is All You Need" launched the transformer revolution, experience suggests that \textit{Attention is Not All You Need} for Software Engineeringâ€”robust systems also require the scaffolding of verification, convergence, and bounded autonomy. Just as CFEngine treats infrastructure as eventually consistent rather than immediately correct, LLM outputs are treated in this framework as eventually valid through iterative refinement.

This work does not propose a revolutionary new algorithm, but rather a formalization of these emerging architectural patterns. The contribution is a theoretical grounding of these heuristicsâ€”providing the vocabulary, convergence guarantees, and formal reasoning framework required to transform ad-hoc "guardrails" into rigorous engineering disciplines.
...
Building on goal-based agent frameworks \cite{shen2014goalbasedagents}, this work introduces a mechanism to externalize reasoning. The definition of an action is extended to utilize guard functionsâ€”not merely as gates, but as active postcondition validators that project the LLM's internal generation process onto a verifiable external state.
...
\begin{itemize}
    ...
    \item \textbf{Atomic Composition:} Generation and verification are treated as a single transactional unitâ€”an "Atomic Action Pair"â€”ensuring that invalid content never pollutes the workflow state.
\end{itemize}
...
\begin{remark}[Workflow Memoization]
\label{rem:memoization}
The executor MAY implement memoization by storing specification hashes in $\mathcal{R}$ metadata. For identical $\Psi$, the cached result is semantically equivalent to re-execution:
\[
\text{hash}(\Psi) = \mathcal{R}.\text{meta}(\texttt{"spec\_hash"}) \implies \text{Execute}(\Psi) \equiv \mathcal{R}.\text{latest}()
\]
This optimization preserves correctness while avoiding redundant computation. Memoization is invalidated when $\Psi$, $\Omega$, or guard implementations change.
\end{remark}
...
\begin{itemize}
    ...
    \item $a_{gen}: \mathcal{C} \to \mathcal{A}$ is the \textbf{Generator} (Execution). It consumes context to produce an artifact or mutate the external environment.
    \item $G: \mathcal{A} \times \mathcal{C} \to \{\top, \bot_{retry}, \bot_{fatal}\} \times \Sigma^*$ is the \textbf{Guard} (Exit Gate). It evaluates the artifact against the context to update the state, requesting refinement ($\bot_{retry}$) or escalation ($\bot_{fatal}$) upon failure.
\end{itemize}
\end{definition}

\begin{remark}[Guard Input Scoping]
While Definition \ref{def:action_pair} provides guards access to the full context $\mathcal{C}$ (to theoretically allow validation against any historical artifact), in practice, well-designed guards should accept only the minimal required inputs. The Workflow configuration is responsible for extracting specific artifacts from $\mathcal{R}$ (via $\mathcal{E}$) and passing them explicitly to the execution runtime, preserving guard simplicity and testability.
\end{remark}

\begin{remark}[Hierarchical Composition \& Semantic Agency]
The generator function $a_{gen}$ is not constrained to be a single atomic inference step. It may be instantiated as an autonomous \textbf{Semantic Agent} (e.g., a ReAct or CoT loop) that operates purely within the stochastic environment state. From the perspective of the Dual-State Workflow, this agentic process is atomic; the Workflow State $S_{w}$ does not track the semantic agent's internal reasoning, only the validity of its final output (or manifest) via the Guard $G$.
\end{remark}

\begin{remark}[Generator Side Effects \& System Stability]
While $a_{gen}$ formally produces an artifact $a$, generators may induce side effects on the environment (e.g., filesystem, APIs). In such cases:
\begin{enumerate}
    \item The artifact $a$ serves as a \textit{receipt} or \textit{manifest} of the \textbf{performed operations}.
    \item The guard $G$ acts as a \textit{sensor}, verifying the environmental state against the manifest.
    \item \textbf{Constraint on Actuation:} While the generator's stochastic output (the plan) may vary between attempts, the \textbf{underlying actuator tools} utilized to execute that plan must be \textbf{idempotent}. That is, the mechanisms for applying changes (e.g., \texttt{mkdir -p}, \texttt{ensure\_file}) must be safe to re-execute without additive errors. This ensures that the Refinement Loop remains stable even if the generator produces redundant instructions during retries.
\end{enumerate}
\end{remark}

\begin{remark}[Side Effect Observability \& Safety]
When $a_{gen}$ induces side effects on the environment, the guard $G$ operates as a post-hoc sensorâ€”it observes the resulting state but cannot prevent or reverse the effects. Policy violations detected by the guard surface as $\bot_{fatal}$ for human review, but recovery from destructive side effects (deletion, corruption) requires external intervention.

Consequently, generators with environmental side effects should prefer \textbf{convergent operations} (e.g., \texttt{mkdir -p}, idempotent writes) where repeated execution or partial failure leaves the system in a recoverable state. Non-convergent operations (deletion, moves) should require human approval \textit{prior to} generator invocation, not post-hoc via guards.
\end{remark}
...
\WHILE{$node \neq None$}
    ...
    \IF{$status = \bot_{retry}$}
        % Refinement Loop: Augment context, retry same node
        \STATE $context \gets \text{AUGMENT}(context, art, feedback)$
        \STATE $retries \gets retries + 1$
        \STATE \textbf{continue}
    \ELSIF{$status = \bot_{fatal}$}
        % Fatal Error: Immediate Termination / Escalation
        \STATE \textbf{return} $(trace, s_w, \text{ESCALATION\_REQUIRED})$
    \ELSE
...
It is acknowledged that these tasks are bounded and synthetic. However, by restricting the problem space, confounding variables are eliminated â€” such as library knowledge or prompt ambiguityâ€”allowing success to be attributed directly to the \textbf{architectural intervention} (the Guard mechanism).

\subsection{Model Qualification}
A foundational premise of this work is that the Dual-State Framework is an architectural multiplier for capability, not a substitute for it. The framework requires a generator capable of \textit{instruction following}â€”specifically, the ability to ingest a guard's error trace and attempt a semantic correction.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Template Normalization:} A unified system prompt is applied across all models, intentionally avoiding model-specific chat templates. This acts as a stress test for "Instruction Following Robustness"â€”models that rely on bespoke control tokens rather than natural language instructions fail the qualification step.
    \end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{model_task_heatmap_guarded.png}
...
This task exhibited the widest variance in guard effectiveness. Top performers achieved substantial gains: \textbf{Yi-Coder (9B)} improved from 56\% to 98\% ($\Delta = +42\text{pp}$, $p < 0.001$, Cohen's $h = 1.17$), while \textbf{StarCoder2 (15B)} reached perfect reliability from a 60\% baseline. The template task proved most discriminating for smaller modelsâ€”\textbf{DeepSeek-Coder (6.7B)} achieved only 18\% guarded success ($\Delta = +14\text{pp}$, $p = 0.051$), suggesting that instruction-following fidelity limits how effectively feedback can be utilized. The sub-3B models showed negligible improvement ($\epsilon \approx 0$), establishing a clear capability threshold.
...
This task exposed a reasoning capability threshold that correlates weakly with parameter count. \textbf{Phi4 (14B)} achieved 98\% baseline, while \textbf{StarCoder2 (15B)} achieved 0\%. The guards proved transformative for StarCoder2: from 0\% baseline to 66\% guarded ($\Delta = +66\text{pp}$, $p < 0.001$, Cohen's $h = 1.90$)â€”the largest effect size observed. This suggests guards can bootstrap reasoning in models that understand the \textit{structure} but fail on \textit{computation}. Sub-3B models showed the capability threshold clearly: \textbf{Qwen2.5-Coder (1.5B)} reached 52\% guarded from 14\% baseline, while \textbf{DeepSeek-Coder (1.3B)} and \textbf{Phi4-Mini (3.8B)} remained at 0\%.
...
\textbf{Key Insightâ€”Task-Specific Qualification:}
...
This process effectively "compiles" the runtime reasoning loopâ€”including the Coach's guidanceâ€”into the model's weights.
...
This enables \textbf{Context Refinement}, where guard feedback is incorporated into subsequent generation attempts. Through Guard Functions, \textbf{Bounded Indeterminacy} is achievedâ€”the architecture does not eliminate the generator's stochastic nature, but confines exploration within logical safety bounds. Additionally, because verification occurs immediately after each generation attempt, the architecture naturally produces immediate, attributable feedbackâ€”a property that may support future integration with reinforcement learning or fine-tuning approaches.
...
As such, the framework's primary contribution is not algorithmic novelty but a formal grounding: providing vocabulary, convergence conditions, and reasoning principles for architectural patterns already emerging in production systemsâ€”extending the tradition of deterministic frameworks for managing unpredictable processes to LLM-based code generation.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        decision/.style={diamond, draw, fill=yellow!10, text width=1.8cm, align=center, aspect=1.5},
        context/.style={rectangle, draw, fill=orange!5, text width=2.0cm, align=center, minimum height=3em},
        coach/.style={hexagon, draw, fill=purple!10, text width=2.2cm, align=center, minimum height=3em, thick}
    ]
    ...
    % --- Boundary Line ---
    \draw[dashed, gray] ($(ctx.east)!0.5!(gen.west)$) coordinate (mid) -- +(0,-8.5) node[above, pos=-0.23, black] {Control Boundary};
    \draw[dashed, gray] (mid) -- +(0,2);
    ...
    % --- SENSING PATH (Artifact -> Guard) ---
    \node (guard) [decision, below=of precond, yshift=-3.5cm] {Guard\\$G(a, C)$};
    \draw[->] (artifact.south) |- node[near start, right, font=\footnotesize] {Senses $a_k$} (guard.east);
    
    % --- CONTEXT INPUT TO GUARD ---
    \draw[->, dotted] (ctx.south) -- ++(0,-2.0) -| (guard.north);

    % --- Logic Paths ---
    \node (success) [block, left=of guard] {Next State\\($s_{w+1}$)};
    \draw[->] (guard) -- node[above] {Pass} (success);

    % --- THE COACH LOOP (Semantic Refinement) ---
    \node (coach) [coach, right=of guard, xshift=1.5cm] {The Coach\\$a_{coach}(\phi)$};
    
    % Fail goes to Coach first
    \draw[->, red, very thick] (guard.east) -- node[above, font=\footnotesize] {Fail ($\perp$)} (coach.west);
    
    % Coach updates Context
    \draw[->, red, very thick, rounded corners] 
        (coach.north) |- ($(ctx.south) + (0.5, -0.5)$) 
        -- node[right, font=\footnotesize, pos=0.0] {Refine $C$} (ctx.south);
    ...
    \caption{The Extended Action Tuple ($A^+$). This diagram integrates the "Coach" (Tier 1 Correction) into the refinement loop. While the Guard remains a deterministic gate, the Coach consumes the error trace $\phi$ to produce semantic guidance, effectively turning sparse failure signals into dense reward shaping before updating the Context.}
    \label{fig:coach_loop}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        thick,
        font=\footnotesize,
        action/.style={rectangle, draw, fill=blue!5, text width=2cm, align=center, rounded corners, minimum height=3em},
        guard/.style={diamond, draw, fill=yellow!10, text width=1.5cm, align=center, aspect=1.5},
        coach/.style={hexagon, draw, fill=purple!10, text width=1.8cm, align=center, minimum height=2.5em},
        artifact/.style={rectangle, draw, fill=white, text width=2cm, align=center, minimum height=2.5em, dashed},
        state/.style={circle, draw, fill=green!10, text width=1.5cm, align=center}
    ]

    % --- PHASE 0: REPRODUCTION ---
    \node (problem) [state, fill=gray!10] {Problem\\Statement};
    
    \node (gen_repro) [action, right=of problem] {Generator\\$g_{repro}$};
    \node (test_art) [artifact, below=of gen_repro] {Repro Test\\(Artifact)};
    
    % G0 Logic
    \node (run_test) [guard, right=of test_art, yshift=0cm] {Exec\\Test};
    \node (sem_check) [coach, right=of run_test] {Semantic\\Check ($G_0^+$)};
    
    % --- PHASE 1: FIX ---
    \node (bug_locked) [state, right=of sem_check] {Bug\\Reproduced};
    \node (gen_fix) [action, right=of bug_locked] {Generator\\$g_{fix}$};
    \node (impl_art) [artifact, below=of gen_fix] {Impl\\(Artifact)};
    \node (g_impl) [guard, left=of impl_art] {Guard\\$G_{impl}$};
    \node (success) [state, left=of g_impl] {Fix\\Committed};

    % --- CONNECTIONS PHASE 0 ---
    \draw[->] (problem) -- (gen_repro);
    \draw[->] (gen_repro) -- (test_art);
    \draw[->] (test_art) -- (run_test);
    
    % G0 Internal Logic
    \draw[->, red] (run_test.north) |- node[above, pos=0.2] {Pass/Error} (gen_repro.east);
    \draw[->] (run_test) -- node[above] {Fail} (sem_check);
    
    % Semantic Check Outcome
    \draw[->] (sem_check) -- node[above] {Relevant} (bug_locked);
    \draw[->, red] (sem_check.north) |- node[above, near start] {Irrelevant} ($(gen_repro.east)+(0,-0.5)$);

    % --- CONNECTIONS PHASE 1 ---
    \draw[->] (bug_locked) -- (gen_fix);
    \draw[->] (gen_fix) -- (impl_art);
    \draw[->] (impl_art) -- (g_impl);
    
    % G_impl Logic (Standard TDD)
    \draw[->] (g_impl) -- node[above] {Pass} (success);
    \draw[->, red] (g_impl.north) |- node[above, near start] {Fail} ($(gen_fix.west)+(0,-0.2)$);

    % --- LABELS ---
    \node[above=0.2cm of gen_repro, font=\bfseries] {Phase 0: Comprehension};
    \node[above=0.2cm of gen_fix, font=\bfseries] {Phase 1: Correction};

    \end{tikzpicture}
    }
    \caption{The Bootstrapping Maintenance Workflow. This illustrates the specialized $G_0$ flow. Note that in Phase 0, a Test Failure is the success condition, provided it passes the Semantic Check (Coach) confirming the failure matches the bug description. Only once the bug is strictly reproduced does the workflow transition to Phase 1 (Correction).}
    \label{fig:bootstrap_flow}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        thick,
        font=\footnotesize,
        action/.style={rectangle, draw, fill=blue!5, text width=1.8cm, align=center, rounded corners, minimum height=3em},
        guard/.style={diamond, draw, fill=yellow!10, text width=1.5cm, align=center, aspect=1.5},
        coach/.style={rectangle, draw, fill=purple!10, text width=1.8cm, align=center, minimum height=3em, rounded corners},
        artifact/.style={rectangle, draw, fill=white, text width=1.8cm, align=center, minimum height=2.5em, dashed},
        state/.style={circle, draw, fill=green!10, text width=1.6cm, align=center, inner sep=2pt},
        start/.style={circle, draw, fill=gray!10, text width=1.6cm, align=center}
    ]

    % --- PHASE 0: COMPREHENSION ---
    \node (problem) [start] {Problem\\Statement};
    \node (gen_repro) [action, right=of problem] {Generator\\$g_{repro}$};
    \node (test_art) [artifact, right=of gen_repro] {Repro Test\\(Artifact)};
    \node (exec_test) [guard, right=of test_art] {Exec\\Test};
    \node (sem_check) [coach, right=of exec_test] {Semantic\\Check ($G_0^+$)};
    \node (bug_locked) [state, right=of sem_check] {Bug\\Reproduced};

    % --- PHASE 1: CORRECTION ---
    % Placing Phase 1 below Phase 0 for better aspect ratio, or continue right if wide page.
    % Here we continue right/down to show flow.
    
    \node (gen_fix) [action, below=of bug_locked, yshift=-0.5cm] {Generator\\$g_{fix}$};
    % Aligning Phase 1 leftwards from the pivot point or keeping it separate? 
    % Let's flow Phase 1 to the LEFT of gen_fix to create a carriage return effect, 
    % OR just start a new row below Phase 0 aligned left.
    
    % Let's do a clean second row aligned with the start for clarity
    \node (gen_fix_viz) [action, below=of gen_repro, yshift=-1.5cm] {Generator\\$g_{fix}$};
    \node (impl_art) [artifact, right=of gen_fix_viz] {Impl\\(Artifact)};
    \node (g_impl) [guard, right=of impl_art] {Guard\\$G_{impl}$};
    \node (success) [state, right=of g_impl] {Fix\\Committed};

    % --- CONNECTING PHASES ---
    % Connect Bug Reproduced (End of Ph0) to Generator Fix (Start of Ph1)
    \draw[->, very thick] (bug_locked.south) |- (gen_fix_viz.west);

    % --- CONNECTIONS PHASE 0 ---
    \draw[->] (problem) -- (gen_repro);
    \draw[->] (gen_repro) -- (test_art);
    \draw[->] (test_art) -- (exec_test);
    \draw[->] (exec_test) -- node[above] {Fail} (sem_check);
    \draw[->] (sem_check) -- node[above] {Relevant} (bug_locked);

    % --- FEEDBACK LOOPS PHASE 0 (Routed ABOVE) ---
    % 1. Exec Test -> Generator (Pass/Error)
    \draw[->, red] (exec_test.north) -- ++(0,0.8) -| node[above, pos=0.25] {Pass/Error} (gen_repro.north);
    
    % 2. Semantic -> Generator (Irrelevant) - Routed slightly lower than the top loop or merged
    \draw[->, red] (sem_check.north) -- ++(0,0.5) -| node[above, pos=0.15] {Irrelevant} ($(gen_repro.north)+(0.5,0)$);

    % --- CONNECTIONS PHASE 1 ---
    \draw[->] (gen_fix_viz) -- (impl_art);
    \draw[->] (impl_art) -- (g_impl);
    \draw[->] (g_impl) -- node[above] {Pass} (success);

    % --- FEEDBACK LOOP PHASE 1 (Routed BELOW) ---
    \draw[->, red] (g_impl.south) -- ++(0,-0.6) -| node[above, pos=0.25] {Fail} (gen_fix_viz.south);

    % --- LABELS ---
    \node[above=0.2cm of gen_repro, font=\bfseries] {Phase 0: Comprehension};
    \node[above=0.2cm of gen_fix_viz, font=\bfseries] {Phase 1: Correction};

    \end{tikzpicture}
    }
    \caption{The Bootstrapping Maintenance Workflow. The diagram is split into two rows for clarity. Top: The Agent attempts to reproduce the bug (Success = Test Failure + Semantic Match). Bottom: The Agent fixes the bug using standard TDD (Success = Test Pass).}
    \label{fig:bootstrap_flow_corrected}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \resizebox{\columnwidth}{!}{
    \begin{tikzpicture}[
        node distance=1.0cm and 1.2cm, % Vertical and Horizontal spacing
        auto,
        >=stealth,
        thick,
        font=\footnotesize,
        % Styles
        action/.style={rectangle, draw, fill=blue!5, text width=1.8cm, align=center, rounded corners, minimum height=3em},
        guard/.style={diamond, draw, fill=yellow!10, text width=1.6cm, align=center, aspect=1.5, inner sep=2pt},
        coach/.style={rectangle, draw, fill=purple!10, text width=1.8cm, align=center, minimum height=3em, rounded corners},
        artifact/.style={rectangle, draw, fill=white, text width=1.8cm, align=center, minimum height=2.5em, dashed},
        state/.style={circle, draw, fill=green!10, text width=1.6cm, align=center, inner sep=2pt},
        start/.style={circle, draw, fill=gray!10, text width=1.6cm, align=center}
    ...
    % =============================================
    % PHASE 1: CORRECTION (BOTTOM ROW)
    % =============================================
    
    % Position Phase 1 strictly below Phase 0, aligned to the left side
    \node (gen_fix) [action, below=of gen_repro, yshift=-1.5cm] {Generator\\$g_{fix}$};
    \node (impl_art) [artifact, right=of gen_fix] {Impl\\(Artifact)};
    \node (g_impl) [guard, right=of impl_art] {Guard\\$G_{impl}$};
    \node (success) [state, right=of g_impl] {Fix\\Committed};
    ...
    % Draws a line from end of Phase 0 to start of Phase 1
    % Uses orthogonal routing (-|) to keep it clean
    \draw[->, very thick] (bug_locked.south) -- ++(0,-0.8) -| ($(gen_fix.west) + (-0.5,0)$) -- (gen_fix.west);
    ...
    % =============================================
    % FEEDBACK LOOPS (THE CRITICAL FIX)
    % =============================================

    % --- Phase 0 Loop 1: Pass/Error (From Exec Test) ---
    % We go UP 1.2cm relative to the node to clear the Artifact node
    \draw[->, red] (exec_test.north) 
        -- ++(0, 1.2) 
        -| node[above, pos=0.25, fill=white, inner sep=2pt] {Pass/Error} (gen_repro.north);
    
    % --- Phase 0 Loop 2: Irrelevant (From Semantic Check) ---
    % We go UP 1.8cm (higher than Loop 1) to avoid overlapping Loop 1
    \draw[->, red] (sem_check.north) 
        -- ++(0, 1.8) 
        -| node[above, pos=0.25, fill=white, inner sep=2pt] {Irrelevant} ($(gen_repro.north) + (0.3,0)$); 
        % Slight x-offset on landing to separate arrowheads

    % --- Phase 1 Loop: Fail (From Guard) ---
    % We go DOWN 1.2cm to clear the Artifact node
    \draw[->, red] (g_impl.south) 
        -- ++(0, -1.2) 
        -| node[below, pos=0.25, fill=white, inner sep=2pt] {Fail} (gen_fix.south);

    % =============================================
    % LABELS
    % =============================================
    \node[above=0.5cm of gen_repro, font=\bfseries] {Phase 0: Comprehension};
    \node[above=0.5cm of gen_fix, font=\bfseries] {Phase 1: Correction};

    \end{tikzpicture}
    }
    \caption{The Bootstrapping Maintenance Workflow. Feedback loops are routed vertically to avoid node collision. Phase 0 (Top) verifies the problem via reproduction. Phase 1 (Bottom) verifies the solution via TDD.}
    \label{fig:bootstrap_flow_final}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        font=\small,
        % --- STYLES ---
        wflow_node/.style={rectangle, draw, fill=blue!5, text width=2.2cm, align=center, rounded corners, minimum height=3em},
        env_node/.style={ellipse, draw, fill=gray!10, text width=2.5cm, align=center, minimum height=3em, dashed},
        artifact/.style={rectangle, draw, fill=white, text width=2.2cm, align=center, minimum height=3em, drop shadow={opacity=0.5}, double copy shadow={shadow xshift=3pt, shadow yshift=-3pt}},
        decision/.style={diamond, draw, fill=yellow!10, text width=1.8cm, align=center, aspect=1.5, inner sep=2pt},
        context/.style={rectangle, draw, fill=orange!5, text width=2.0cm, align=center, minimum height=3em},
        
        % FIX 1: Use 'regular polygon' instead of 'hexagon' (which triggered the 100 errors)
        coach/.style={regular polygon, regular polygon sides=6, draw, fill=purple!10, text width=1.5cm, align=center, minimum height=3em, thick, inner sep=-2pt},
        
        % New Style for Human
        human/.style={circle, draw, fill=red!10, text width=1.8cm, align=center, minimum height=3em, thick},
        
        state_label/.style={font=\bfseries\large, align=center},
        action_pair/.style={draw, dashed, gray, rounded corners, inner sep=15pt, fill=none, line width=1pt},
        group_label/.style={font=\bfseries\footnotesize, color=gray, anchor=north west}
    ]

    % ==========================================================================================
    % PHASE 0: COMPREHENSION (TOP HALF)
    % ==========================================================================================
    
    % --- HEADERS ---
    \node (h_workflow) [state_label] at (0, 1) {Workflow State\\(Deterministic)};
    \node (h_env) [state_label] at (8, 1) {Environment State\\(Stochastic)};
    \draw[dashed, gray] (4.5, 1.5) -- (4.5, -13) node[above, pos=0.0, black] {Control Boundary};

    % --- ROW 1: PRECONDITIONS ---
    \node (p0_start) [wflow_node] at (0, 0) {Start State\\($s_{problem}$)};
    \node (p0_pre) [decision, right=of p0_start, xshift=-0.5cm] {Precond\\$\rho(s_p)$};
    \node (p0_ctx) [context, right=of p0_pre, xshift=-0.5cm] {Context\\$C_{issue}$};
    
    % --- ROW 1: GENERATION (Environment) ---
    \node (p0_gen) [env_node] at (8, 0) {Generator\\$g_{repro}(C)$};
    \node (p0_art) [artifact, below=of p0_gen, yshift=-0.5cm] {Repro Test\\$a_{test}$};

    % --- ROW 2: GUARD LOGIC (Workflow) ---
    \node (p0_guard_exec) [decision] at (2, -3) {Exec Test\\(Deterministic)};
    \node (p0_guard_sem) [coach, left=of p0_guard_exec, xshift=-0.5cm] {Semantic Check\\$G_0^+$};
    \node (p0_next) [wflow_node, left=of p0_guard_sem, xshift=-0.5cm, fill=green!10] {Next State\\($s_{bug\_locked}$)};

    % --- PHASE 0 CONNECTIONS ---
    \draw[->] (p0_start) -- (p0_pre);
    \draw[->] (p0_pre) -- node[above] {True} (p0_ctx);
    \draw[->] (p0_ctx) -- (p0_gen);
    \draw[->] (p0_gen) -- node[right] {Produces} (p0_art);
    
    % Sensing
    \draw[->] (p0_art.south) |- (4.5, -3) -- node[above, pos=0.5] {Senses} (p0_guard_exec.east);
    
    % Exec Logic (Inverted for Repro)
    \draw[->] (p0_guard_exec) -- node[above] {Fail (1)} (p0_guard_sem);
    
    % Semantic Logic
    \draw[->] (p0_guard_sem) -- node[above] {Match} (p0_next);

    % Refinement Loops (Red)
    \draw[->, red, thick] (p0_guard_exec.north) |- node[right, pos=0.2] {Pass/Error ($\perp$)} ($(p0_ctx.south)+(0.5,0)$);
    \draw[->, red, thick] (p0_guard_sem.north) |- node[left, pos=0.2] {Irrelevant ($\perp$)} ($(p0_ctx.south)+(-0.5,0)$);


    % ==========================================================================================
    % PHASE 1: CORRECTION (BOTTOM HALF)
    % ==========================================================================================

    % Visual Separator
    \draw[dotted, thick, gray] (-2, -5) -- (10, -5);
    \node [anchor=west, font=\itshape, gray] at (-2, -4.8) {Transition: Problem Locked $\to$ Solution Space};

    % --- ROW 3: PRECONDITIONS ---
    \node (p1_start) [wflow_node] at (0, -6.5) {Start State\\($s_{bug\_locked}$)};
    \draw[->, very thick, double] (p0_next.south) -- ++(0,-0.5) -| (p1_start.north);

    \node (p1_ctx) [context] at (4.5, -6.5) {Context\\$C_{fix}$};
    \draw[->] (p1_start) -- node[above] {Load} (p1_ctx);

    % --- ROW 3: GENERATION ---
    \node (p1_gen) [env_node] at (8, -6.5) {Generator\\$g_{fix}(C)$};
    \node (p1_art) [artifact, below=of p1_gen, yshift=-0.5cm] {Impl\\$a_{code}$};

    % --- ROW 4: GUARD LOGIC (Standard TDD) ---
    \node (p1_guard) [decision] at (4.5, -9.5) {Guard\\$G_{impl}$};
    \node (p1_next) [wflow_node, left=of p1_guard, xshift=-1.5cm, fill=green!10] {Goal State\\($s_{fixed}$)};

    % --- NEW: HUMAN SUPERVISOR ---
    \node (human) [human] at (4.5, -12.5) {Human\\Supervisor};

    % --- PHASE 1 CONNECTIONS ---
    \draw[->] (p1_ctx) -- (p1_gen);
    \draw[->] (p1_gen) -- node[right] {Produces} (p1_art);
    
    % Sensing
    \draw[->] (p1_art.south) |- (8, -9.5) -- node[above] {Senses} (p1_guard.east);
    
    % Logic
    \draw[->] (p1_guard) -- node[above] {Pass ($\top$)} (p1_next);
    
    % Refinement Loop (Standard)
    \draw[->, red, thick] (p1_guard.north) -- node[right] {Fail ($\perp$)} (p1_ctx.south);

    % --- FATAL / HUMAN CONNECTIONS ---
    % 1. Fatal Signal from Guard to Human
    \draw[->, red, very thick] (p1_guard.south) -- node[right, font=\bfseries] {Fatal ($\perp_{fatal}$)} (human.north);

    % 2. Audit/Review from Human to Artifact
    \draw[->, blue, dashed, thick] (human.east) -| node[pos=0.25, below, font=\itshape] {Audits / Reviews} (p1_art.south);


    % ==========================================================================================
    % VISUAL GROUPING (BOXES & LINKS)
    % ==========================================================================================
    
    \begin{scope}[on background layer]
        % 1. Phase 0 Action Pair Box
        \node [action_pair, fit=(p0_gen) (p0_art) (p0_guard_exec)] (box_p0) {};
        \node [group_label] at (box_p0.north west) {1. Characterization Test Pair};

        % 3. The Coach Box (Semantic Check)
        \node [action_pair, fit=(p0_guard_sem), inner sep=5pt, border color=purple!50, dashed] (box_coach) {};
        \node [group_label, color=purple] at (box_coach.north west) {3. The Coach};

        % 2. Phase 1 Action Pair Box
        \node [action_pair, fit=(p1_gen) (p1_art) (p1_guard)] (box_p1) {};
        \node [group_label] at (box_p1.north west) {2. TDD Action Pair};
    \end{scope}

    % 4. Dotted Link from Ph0 Artifact to Ph1 Guard
    \draw[->, dotted, very thick, blue] (p0_art.east) 
        -- ++(0.5, 0) 
        |- node[pos=0.25, right, font=\bfseries\footnotesize, blue, align=left, fill=white] {4. Becomes\\Spec} 
        (p1_guard.east);

    % Labels for Phases (FIX 2: align=center prevents LR mode error)
    \node [rotate=90, font=\bfseries\large, anchor=center, align=center] at (-2.5, -1.5) {PHASE 0:\\Comprehension};
    \node [rotate=90, font=\bfseries\large, anchor=center, align=center] at (-2.5, -8) {PHASE 1:\\Correction};

    \end{tikzpicture}
    }
    \caption{The Dual-State Bootstrapping Workflow (Annotated). Now includes the Human Supervisor loop. When the Phase 1 Guard emits a Fatal signal (e.g., infinite regression), the Human intervenes to audit the Artifact History and manually reset the Context.}
    \label{fig:dual_state_bootstrap_human}
\end{figure}
 ...
 \begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        decision/.style={diamond, draw, fill=yellow!10, text width=1.8cm, align=center, aspect=1.5, inner sep=2pt},
        context/.style={rectangle, draw, fill=orange!5, text width=2.0cm, align=center, minimum height=3em},
        human/.style={circle, draw, fill=red!10, text width=2.0cm, align=center, minimum height=3em, thick, inner sep=2pt},
        state_label/.style={font=\bfseries\large, align=center}
    ]
    ...
    % --- Headers & Boundary ---
    \node (h_workflow) [state_label] at (0, 1.5) {Workflow State\\(Deterministic)};
    \node (h_env) [state_label] at (8, 1.5) {Environment State\\(Stochastic)};
    \draw[dashed, gray] (4.5, 2.0) -- (4.5, -8.0) node[above, pos=0.0, black] {Control Boundary};

    % --- Main Flow (Row 1) ---
    \node (start) [wflow_node] at (0, 0) {Start State\\($s_w$)};
    \node (precond) [decision, right=of start, xshift=-0.5cm] {Precond\\$\rho(s_w)$};
    \node (ctx) [context, right=of precond, xshift=-0.5cm] {Context\\$C$};
    \node (gen) [env_node] at (8, 0) {Generator\\$a_{gen}(C)$};
    
    % --- Artifacts (Row 2) ---
    \node (artifact) [artifact, below=of gen, yshift=-0.5cm] {Artifact History\\$\mathcal{R}$};

    % --- Validation (Row 3) ---
    \node (guard) [decision] at (4.5, -4.5) {Guard\\$G(a, C)$};
    \node (next) [wflow_node, left=of guard, xshift=-2.0cm, fill=green!10] {Next State\\($s_{w+1}$)};

    % --- THE HUMAN SUPERVISOR (Row 4) ---
    \node (human) [human] at (4.5, -7.5) {Human\\Supervisor};
    ...
    \draw[->] (start) -- (precond);
    \draw[->] (precond) -- node[above] {True} (ctx);
    \draw[->] (ctx) -- (gen);
    \draw[->] (gen) -- node[right] {Produces} (artifact);
    
    % --- Sensing ---
    \draw[->] (artifact.south) |- (8, -4.5) -- node[above, pos=0.5] {Senses} (guard.east);
    
    % --- Outcomes ---
    % 1. Pass
    \draw[->] (guard) -- node[above] {Pass ($\top$)} (next);
    
    % 2. Retry Loop (Standard)
    \draw[->, red] (guard.north) 
        -- ++(0, 0.5) 
        -| node[right, pos=0.25, fill=white] {Retry ($\perp$)} ($(ctx.south) + (0.5, 0)$);

    % 3. FATAL SIGNAL (The New Path)
    \draw[->, red, very thick] (guard.south) -- node[right] {Fatal ($\perp_{fatal}$)} (human.north);
    ...
    \draw[->, blue, dashed, thick] (human.east) 
        -- ++(0.5, 0) 
        |- node[right, pos=0.25, align=left] {\textit{Audits}\\Artifacts} ($(artifact.south) + (0.5, 0)$);

    % 2. Manual Intervention (Write)
    \draw[->, blue, thick] (human.west) 
        -- ++(-2.5, 0) 
        |- node[right, pos=0.25, align=left] {\textit{Manual}\\Recovery} (ctx.west);

    \end{tikzpicture}
    }
    \caption{The Human-in-the-Loop Action Pair. When the Guard emits a Fatal signal ($\perp_{fatal}$), control is escalated to the Human Supervisor. The Human audits the immutable Artifact History ($\mathcal{R}$) to diagnose the failure and performs a manual recovery by injecting a corrected Context ($C$), effectively resolving the deadlock that the autonomous loop could not handle.}
    \label{fig:human_loop}
\end{figure}
