...
Current approaches to AI coding agents appear to blur the lines between the Large Language Model (LLM) and the agent itself, asking the LLM to make decisions best left to deterministic processes. This leads to systems prone to stochastic failures such as gaming unit tests or hallucinating syntax. Drawing on established software engineering practices that provide deterministic frameworks for managing unpredictable processes, this paper proposes setting the control boundary such that the LLM is treated as a component of the environment rather than the decision-making agent.
...
Additionally, the architecture provides a potential approach to the Credit Assignment Problem inherent in LLM training. By enforcing immediate verification the framework naturally generates dense reward signals attributed to specific generation attempts, rather than the sparse rewards typical of end-to-end generation. These verified traces can support both online reinforcement learning and offline supervised fine-tuning (e.g., via LoRA). Over time, this could theoretically reduce retry rates from the observed 2.1× toward 1.0× as models internalize domain-specific constraints.
...
\subsection{Safety as a Workflow Property}
Because the outputs of stochastic generators cannot be predicted without execution, reliance on internal model alignment alone may be insufficient for high-stakes domains. This framework treats safety as a property of the workflow rather than solely an attribute of the model—decoupling generation from validation allows external verification to proceed independently of the generator's internal behavior.
...
This enables \textbf{Context Refinement}, where guard feedback is incorporated into subsequent generation attempts. Through Guard Functions, \textbf{Bounded Indeterminacy} is achieved—the architecture does not eliminate the generator's stochastic nature, but confines exploration within logical safety bounds.

Additionally, because verification occurs immediately after each generation attempt, the architecture naturally produces dense, attributable feedback—a property that may support future integration with reinforcement learning or fine-tuning approaches.

The architecture also enforces security properties structurally—a SAST check implemented as a Guard Function is mandatory, unlike providing a tool that an LLM-based agent may or may not invoke.

Experimental validation across 13 models and three diagnostic tasks indicates that the framework can substantially improve reliability for qualified instruction-following models, with observed gains of up to 66 percentage points at modest computational overhead (1.2--2.1$\times$). The results also suggest that guards cannot compensate for fundamental reasoning deficits—models must possess sufficient capability to use verification feedback effectively.

The framework's primary contribution is not algorithmic novelty but a formal grounding: providing vocabulary, convergence conditions, and reasoning principles for architectural patterns already emerging in production systems—extending the tradition of deterministic frameworks for managing unpredictable processes to LLM-based code generation.

Limitations include guard design overhead, context window saturation under high retry counts, and the assumption of static specifications. Future directions include continuous learning from verification traces, dynamic guard selection, and application to multi-agent coordination through shared observable state.

\section*{Acknowledgments}

I would like to thank \textbf{Mark Burgess}\footnote{Link} and \textbf{Ray Myers}\footnote{Link} for their guidance on formalization and pointing me in sensible directions, \textbf{Joanna Bryson}\footnote{Link} for her insights on AI ethics. I am also grateful to \textbf{Professor Jeremy Scerri}\footnote{Link} and \textbf{Jessica Sciammarelli}\footnote{Link} for their support and for opening critical pathways in my broader learning.

Special thanks to \textbf{Lio}\footnote{Link} for providing the hardware resources, and to the \textbf{Cohere Labs} community\footnote{Link}, led by \textbf{Madeline}\footnote{Link}, for providing a welcoming and energizing environment for this research.

I am also grateful to the software engineering community leaders who have championed the practices central to this work: \textbf{Bryan Finster}\footnote{Link}, \textbf{Tracy Bannon}\footnote{Link}, \textbf{Patrik Debois}\footnote{Link}, \textbf{Matthew Sheldon}\footnote{Link}, \textbf{Vaz}\footnote{Link}, and \textbf{Rob Burrows}\footnote{Link}.

Finally, thank you to \textbf{Erwan Kennedy}\footnote{Link} and \textbf{David Nell}\footnote{Link} for being there at the start, and to my peers for their feedback and support: \textbf{Hugo Mirelles}\footnote{Link}, \textbf{Natlia}\footnote{Link}, \textbf{Olgo}\footnote{Link}, and \textbf{Philip}\footnote{Link}.
